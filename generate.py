...
/* A card has no frame until it comes near the viewport, and loses it once it is
   ...
   while it is worth showing. */
function loadShot(shot){
  if (live.has(shot)) return;
  const inner = document.createElement('div');
  inner.className = 'shot-inner';
  const f = document.createElement('iframe');
  f.className = 'shot-frame';
  f.setAttribute('scrolling', 'no');
  f.setAttribute('tabindex', '-1');
  f.setAttribute('aria-hidden', 'true');
  f.addEventListener('load', () => shot.classList.add('loaded'), { once: true });
  inner.appendChild(f);
  shot.appendChild(inner);
  fitShot(inner, shot);
  f.src = shot.dataset.url;
  live.add(shot);
}
function freeShot(shot){
  if (!live.has(shot)) return;
  const inner = shot.querySelector('.shot-inner');
...
let shotEls = [];
let shotTick = false;
function shotDist(el){
  const r = el.getBoundingClientRect();
  if (r.bottom < 0) return -r.bottom;
  return r.top > innerHeight ? r.top - innerHeight : 0;
}
/* cards are stacked in document order, so the live ones are one slice: find it
   by bisection and leave the other 1400 alone */
function updateShots(){
  if (!shotEls.length) return;
  let lo = 0, hi = shotEls.length - 1, start = shotEls.length;
  while (lo <= hi){
    const mid = (lo + hi) >> 1;
    if (shotEls[mid].getBoundingClientRect().bottom < -FAR) lo = mid + 1;
    else { start = mid; hi = mid - 1; }
  }
  for (let i = start; i < shotEls.length; i++){
    const s = shotEls[i];
    if (s.getBoundingClientRect().top > innerHeight + FAR) break;
    if (shotDist(s) <= NEAR) loadShot(s);
  }
  live.forEach(s => { if (shotDist(s) > FAR) freeShot(s); });
}
function queueShots(){
  if (shotTick) return;
  shotTick = true;
  requestAnimationFrame(() => { shotTick = false; updateShots(); });
}
function observeShots(){
  live.forEach(freeShot);
  shotEls = Array.from(document.querySelectorAll('.pg-shot'));
  queueShots();
}
addEventListener('scroll', queueShots, { passive: true });
addEventListener('resize', () => {
  document.querySelectorAll('.shot-inner').forEach(inner => {
    const shot = inner.closest('.pg-shot');
    if (shot) fitShot(inner, shot);
  });
  queueShots();
});
...
<p>One handcrafted landing page for each of __TOTAL__ fictional products across __SECTORS_N__ industries. Search, filter, sort — every card previews its page live, and a click opens it in a device-sized viewer.</p>
